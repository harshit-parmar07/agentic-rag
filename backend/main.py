...
import agent
from vectorstore import process_pdf_and_store

@asynccontextmanager
async def lifespan(app: FastAPI):
    # Initialize connection pool/connection and build agent on server startup
    agent.init_agent()
    yield
    # Clean up SQLite connection pool cleanly on shutdown
    agent.close_agent()
...
async def upload_document(file: UploadFile = File(...)):
    ...
    try:
        total_chunks_added = await asyncio.to_thread(process_pdf_and_store, temp_file_path)
        ...
        print(f"Validation Error processing PDF document: {e}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=str(e)
        )
...
async def chat_with_agent(request: QueryRequest):
    ...
    try:
        ...
        for i, s in enumerate(agent.rag_agent.stream(inputs, config=config)):
        ...
        print(f"Validation Error during agent invocation: {e}")
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except groq.AuthenticationError as e:
        print(f"Groq Authentication Error: {e}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="Invalid Groq API Key. Please verify your GROQ_API_KEY in config.env."
        )
    except Exception as e:
        import traceback
        traceback.print_exc()
