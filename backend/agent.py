...
class RewrittenQuery(BaseModel):
    standalone_query: str = Field(..., description="The contextually resolved, standalone query.")

# --- LLM instances with structured output where needed ---
os.environ["GROQ_API_KEY"] = GROQ_API_KEY or "dummy-key"

router_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0).with_structured_output(RouteDecision)
judge_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0).with_structured_output(RagJudge, method="json_mode")
rewriter_llm = ChatGroq(model="llama-3.1-8b-instant", temperature=0).with_structured_output(RewrittenQuery)
...
# --- Node 0: query rewriter ---
def query_rewriter_node(state: AgentState) -> AgentState:
    print("\n--- Entering query_rewriter_node ---")
    messages = state.get("messages", [])
    
    if len(messages) <= 1:
        query = next((m.content for m in reversed(messages) if isinstance(m, HumanMessage)), "")
        print("Bypassing rewriter (first message).")
        return {**state, "standalone_query": query}
        
    query = next((m.content for m in reversed(messages) if isinstance(m, HumanMessage)), "")
    
    system_prompt = (
        "Given the following conversation and a follow-up question, "
        "rephrase the follow-up question to be a standalone query that captures all relevant context. "
        "If it is already standalone, return it unchanged. Do not answer the question."
    )
    
    llm_messages = [("system", system_prompt)] + messages[:-1] + [("user", query)]
    
    try:
        result = rewriter_llm.invoke(llm_messages)
        standalone_query = result.standalone_query
    except Exception as e:
        print(f"Rewriter LLM error: {e}. Falling back to raw query.")
        standalone_query = query
        
    print(f"Rewritten query: {standalone_query}")
    print("--- Exiting query_rewriter_node ---")
...
def router_node(state: AgentState,config : RunnableConfig) -> AgentState:
    ...
    print(f"Messages in state: {state.get('messages', [])}")
    ...
    system_prompt += (
        ...
        "\n- User: 'What are the treatments for diabetes?' -> Route: 'rag' (Factual knowledge, likely in KB)."
...
def rag_node(state: AgentState,config:RunnableConfig) -> AgentState:
    ...
    judge_messages = [
        ("system", (
            ...
            "You MUST return a JSON object with a single key 'sufficient' set to true or false. Do not include any other keys.\n"
            "JSON Format:\n"
            "{\n"
            "  \"sufficient\": true\n"
            "}"
...
def web_node(state: AgentState, config: RunnableConfig) -> AgentState:
    ...
    web_search_enabled = config.get("configurable", {}).get("web_search_enabled", True)
    print(f"Web node received web search info: {web_search_enabled}")
    ...
    try:
        ...
        snippets = "\n\n".join(f"Title: {item['title']}\nContent: {item['snippet']}" for item in extracted)
    except Exception as e:
        print(f"Web Error: {e}. Proceeding to answer with limited info.")
        snippets = ""
...
def answer_node(state: AgentState) -> AgentState:
    ...
    user_q = state.get("standalone_query", "")
...
Provide a helpful, accurate, and concise response based on the available information."""
    ...
    # Prepend conversation history to let LLM leverage context from past messages
    system_prompt = f"You are a helpful assistant. Use the following context if relevant:\n{context}"
    llm_messages = [("system", system_prompt)] + state["messages"][:-1] + [HumanMessage(content=prompt)]
    
    ans = answer_llm.invoke(llm_messages).content
...
def build_agent(checkpointer):
    """Builds and compiles the LangGraph agent with a checkpointer."""
...
def init_agent():
    """Initializes database pool/connection and compiles the agent graph on startup."""
    global db_conn, checkpointer, rag_agent
    db_path = os.path.join(os.path.dirname(__file__), "checkpoints.sqlite")
    print(f"Initializing SQLite checkpointer at: {db_path}...")
    
    # Establish connection with thread-safety disabled for SQLite multi-thread access
    db_conn = sqlite3.connect(db_path, check_same_thread=False)
    checkpointer = SqliteSaver(db_conn)
    
    # Run the setup script to automatically execute native schema creation
    checkpointer.setup()
    
    # Compile the state graph using the persistent checkpointer
    rag_agent = build_agent(checkpointer)
    print("Persistent SQLite checkpointer initialization and schema setup complete.")

def close_agent():
    """Closes checkpointer database connection pool/connection cleanly on server shutdown."""
    global db_conn
    if db_conn:
        print("Closing checkpointer SQLite connection...")
        db_conn.close()
        db_conn = None
